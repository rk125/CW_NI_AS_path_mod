...
def main():
    ...
    parser.add_argument("--ASNs",
                        help="AS numbers to add to policy in form 111|222|333")
    ...
    # read
    exrest.uri = "/api/beta/config/export"
    exrest.method = "GET"
    response=exrest.run() #response as dictionary
    # modify
    for policy in response["alarmPolicies"]:
      if "PREFIX" in policy["name"]:
        policy["asPathPattern"]="(?<! ** ("+args.ASNs+") **)"
        # print (i["name"], i["asPathPattern"])
    # print(json.dumps(response, indent=2))

    # write back
    exrest.payload = json.dumps(response) #payload as text
    exrest.uri = "/api/beta/config/import"
    exrest.method = "POST"
    response=exrest.run()
    print ("AS path mod done")
